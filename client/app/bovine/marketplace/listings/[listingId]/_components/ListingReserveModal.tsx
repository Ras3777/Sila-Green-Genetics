...
interface ListingReserveModalProps {
  isOpen: boolean;
  onClose: () => void;
  reserveDeposit: number;
  setReserveDeposit: (deposit: number) => void;
  onSubmit: (e: React.FormEvent) => void;
}

export function ListingReserveModal({
  isOpen,
  onClose,
  reserveDeposit,
  setReserveDeposit,
  onSubmit,
}: ListingReserveModalProps) {
      ...
      <div className="bg-white rounded-3xl max-w-md w-full p-6 border border-stone-200 shadow-2xl space-y-4">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <button type="submit" className="px-4 py-2 text-sm font-semibold text-white bg-emerald-800 rounded-xl cursor-pointer">Confirm Hold</button>
          </div>
        </form>
      </div>
    </div>
