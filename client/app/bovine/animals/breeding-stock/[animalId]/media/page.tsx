...
import { ArrowLeft, Award, Camera, Layers } from 'lucide-react';
import { useBovine } from '@/lib/bovine-store';
import AnimalMediaGallery from '@/components/bovine/media/AnimalMediaGallery';

export default function BreedingStockMediaPage({
  params,
}: {
  ...
  if (!animal) {
    return (
      <div className="p-8 max-w-4xl mx-auto text-center space-y-4">
        ...
        <p className="text-xs text-stone-500">No active breeding stock animal matches ID &ldquo;{animalId}&rdquo;.</p>
        <Link
    ...
    <div className="space-y-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="flex items-center space-x-2 text-xs text-stone-500">
        ...
        <span className="font-semibold text-stone-900 flex items-center gap-1.5">
          ...
          Breeding Conformation &amp; Media Dossier
        </span>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-stone-200 shadow-2xs p-6 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        ...
        <div>
          ...
          >
            <ArrowLeft className="w-3.5 h-3.5 mr-1" />
            <span>Back to Breeding Profile</span>
          </Link>
        </div>
      </div>
      ...
      <AnimalMediaGallery
        ...
        backHref={`/bovine/animals/breeding-stock/${animalId}`}
        hideHeroBanner={true}
      />
    </div>
  );
